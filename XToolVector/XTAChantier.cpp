...
bool XTACliche::ReadAttributes(std::vector<std::string>& V)
{
	...
	switch(m_Style) {
		...
		case MultiSpectral : style = "NumÃ©rique multispectral"; break;
		case DoubleFauchee : style = "NumÃ©rique double fauchée"; break;
		case Argentique : style = "Argentique"; break;
		case AvantArriere : style = "NumÃ©rique avant arrière"; break;
	}
	V.push_back("Style");
	V.push_back(style);
	...
	if (m_Style != Argentique) {
		V.push_back("RÃ©solution moyenne");
		data << m_dResolMoy;
	} else {
	...
	// Cap
	data << Cap();//cap en degrés
	V.push_back("Cap (en degrÃ©s)");
	V.push_back(data.str());
	data.str("");

	V.push_back("Hauteur du soleil (en degrÃ©s)");
	data << (m_dSunHeight* 180.) / XPI;
	V.push_back(data.str());
	data.str("");

	V.push_back("ClichÃ© en ZI");
	if (m_bZI)
		V.push_back("Oui");
	else
		V.push_back("Non");
