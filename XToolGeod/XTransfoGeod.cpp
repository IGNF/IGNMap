...
bool XTransfoGeod::SetGeoref(double* xmin, double* ymax, double* gsd, unsigned short* epsg)
{
  ...
  *ymax = m_Ff.Ymax;
  *gsd = m_dResol;
  *epsg = (unsigned short)XGeoProjection::EPSGCode(m_Geod->StartProjection());
  return true;
}
...
// Transformation image d'arrivee -> image de depart
//-----------------------------------------------------------------------------
void XTransfoGeodZoom::Direct(double x, double y, double* u, double* v)
{
  double xi, yi, xf, yf;

  xi = x * m_dResolF + m_Ff.Xmin;
  yi = m_Ff.Ymax - y * m_dResolF;

  m_Geod->Convert(xi, yi, xf, yf);

  *u = (xf - m_Fi.Xmin) / m_dResolI;
  *v = (m_Fi.Ymax - yf) / m_dResolI;
  return;
}
...
// Dimension de l'image de sortie
//-----------------------------------------------------------------------------
void XTransfoGeodZoom::Dimension(int /*w*/, int /*h*/, int* wout, int* hout)
{
  *wout = XRint(m_Ff.Width() / m_dResolF);
  *hout = XRint(m_Ff.Height() / m_dResolF);
}

//-----------------------------------------------------------------------------
// Georefencement de l'image resultante
//-----------------------------------------------------------------------------
bool XTransfoGeodZoom::SetGeoref(double* xmin, double* ymax, double* gsd, unsigned short* epsg)
{
  *xmin = m_Ff.Xmin;
  *ymax = m_Ff.Ymax;
  *gsd = m_dResolF;
...
// Auto-calibration pour trouver le pas de la grille d'interpolation
//-----------------------------------------------------------------------------
bool XTransfoGeodZoomInterpol::AutoCalibration()
{
  if (m_Ff.IsEmpty())
    return false;
  if (m_dResolF <= 0.)
...
// Calcul de l'erreur pour un rectangle
//-----------------------------------------------------------------------------
double XTransfoGeodZoomInterpol::ErrorFrame(XFrame F)
{
  double xa, ya, xb, yb, xc, yc, xd, yd, xp, yp;
...
// Transformation image d'arrivee -> image de depart
//-----------------------------------------------------------------------------
void XTransfoGeodZoomInterpol::Direct(double x, double y, double* u, double* v)
{
  double xf, yf, epsilon = m_dResolF * 0.01;
  m_P.X = x * m_dResolF + m_Ff.Xmin;
  m_P.Y = m_Ff.Ymax - y * m_dResolF;

  double ratio;
  if (fabs(m_P.Y - m_Ai.Y) < m_dResolF - epsilon) {
    if ((m_P.X >= m_Ai.X) && (m_P.X <= m_Bi.X)) {	// P entre A et B
      ratio = (m_P.X - m_Ai.X) / (m_Bi.X - m_Ai.X);
      xf = m_Af.X + ratio * (m_Bf.X - m_Af.X);
      yf = m_Af.Y + ratio * (m_Bf.Y - m_Af.Y);
